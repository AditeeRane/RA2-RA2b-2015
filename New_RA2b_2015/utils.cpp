...
TString Utils::TauPtEta_name(unsigned int ptBin) {
  TauResponse_checkPtBin(ptBin);
  TString name = "hGenPtEta_";
  name += ptBin;
  return name;
}
...
TH1D *  Utils::GetBinxProj(TH2D * h2tau_phi, int binx){
  TH1D * ProjY=h2tau_phi->ProjectionY("angularTemplate",binx,binx,"");
  return ProjY;
}

double Utils::GetTotProjArea(TH1D * ProjY){
  double Dphi_area=0;
  double bin_area=0;
  //TH1D * ProjY=h2tau_phi->ProjectionY("angularTemplate",binx,binx,"");
  for(int i=1;i<=10;i++){
    bin_area = ProjY->GetBinContent(i)*ProjY->GetBinWidth(i);
    Dphi_area +=bin_area;
  }
  return Dphi_area;
}

double Utils::GetBinProjArea(TH1D * ProjY,int p){
  //  TH1D * ProjY=h2tau_phi->ProjectionY("angularTemplate",binx,binx,"");
  double bin_p=ProjY->GetBinContent(p)*ProjY->GetBinWidth(p);
  return bin_p;
}
