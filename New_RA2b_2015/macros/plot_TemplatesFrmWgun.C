...
void plot_TemplatesFrmWgun(){
  ...
  TFile *file_TauGan        = new TFile("/uscms_data/d3/arane/work/WGun_Jan30/CMSSW_8_0_25/src/TreeMaker/Production/test/hist_taugun_aditee.root","R");
  TH1D * thist;
  catLeg1->SetHeader("p_{T}(#tau^{gen})");

  for(int i=0;i<4;i++){
    sprintf(tempname,"hTauResp_%d",i);
    thist = (TH1D*)file_TauGan->Get(tempname)->Clone();
    //sprintf(tempname2,"hTauResp_%d_AB",i);
    //thist->SetName(tempname);
    ...
    if(i==0){
      ...
      thist->GetXaxis()->SetTitleOffset(1.1);
    ...
    //KH if (i==0){
    thist->SetTitle(";p_{T}(visible)/p_{T}(#tau^{gen});Arbitrary unit");
    //    thist->GetYaxis()->SetTitle("Arbitrary unit");
    //thist->GetXaxis()->SetTitle("p_{T}(visible)/p_{T}(#tau^{gen})");
...
*/  
  ...
  c1->Print("Plot_WgunTemplate.pdf");
  c1->Print("Plot_WgunTemplate.png");
