...
void Plot_closure(string cutname="delphi", string histname="NBtag",string sample="stacked",
  ...
  else sprintf(tempname,"%sGenInfo_HadTauEstimation_%s.root",elogForExp.c_str(),sample.c_str());
  ...
  else sprintf(tempname,"%sHadTauEstimation_%s.root",elogForPre.c_str(),sample.c_str());
  ...
  //}
  {
    CMS_lumi(canvas,iPeriod,iPos );   // writing the lumi information and the CMS "logo"
  }
  canvas->Update();
  canvas->RedrawAxis();
