...
void compareResponse(char* WJetTTbarMC, char* Wgun, char* histOne, char *histTwo, bool logy, bool logx, bool overlay,bool normalized){
  ...
  //TH1F *hDiff = new TH1F("hDiff","Difference between tau and mu B mistag rate",100,0.,1000.);    
  int Nbins=100;
  ...
  for(int j=1;j<=Nbins;j++){
    double TauRate=h1->GetBinContent(j);
    double MuRate=h2->GetBinContent(j);
    double diff=TauRate-MuRate;
    double e1 =  h1->GetBinError(j);
    double e2 = h2->GetBinError(j) ;
    std::cout << h1->GetBinError(j) << " " << h2->GetBinError(j) << " " << sqrt(e1*e1+e2*e2) << " " << hDiff->GetBinError(j) << " " << hDiff->GetBinContent(j) << " " << sqrt(hDiff->GetBinContent(j)) << std::endl;
    //hDiff->SetBinContent(j,diff);
  }
  ...
  TFile* xf = new TFile("hist_bRateDiff_TauVsMu_08Dec.root","RECREATE");
  hDiff->Write();
  xf->Close();
